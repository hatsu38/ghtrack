...
**v0.1.0 prototype**: collects job / step duration of a workflow run, appends it to `data/<workflow-file-basename>.json` on the `gh-pages` branch (one file per workflow), and bundles a Chart.js-powered `index.html` at the root of the same branch. Once GitHub Pages is enabled for the repository, the time-series charts are visible in the browser.

**🌐 Live demo**: https://hatsu38.github.io/ghtrack/  ← this repo's own dogfood

## Usage

Add this Action to the end of any workflow in the repository. If the `gh-pages` branch does not exist, an orphan branch is created automatically on the first run. Each workflow's data is stored in `data/<workflow-file-basename>.json` by default, so you can drop the Action into multiple workflows without configuring anything.
...
| `data-file-path` | `data/<workflow-file-basename>.json` | JSON file path inside the branch. Defaults to the calling workflow file's basename (e.g. `.github/workflows/test.yml` → `data/test.json`) |
...
Each workflow automatically stores its data under `data/<workflow-file-basename>.json` (e.g. `.github/workflows/unit-test.yml` → `data/unit-test.json`). You can drop the Action into multiple workflows without configuring `data-file-path` and the dashboard will display each workflow as a separate dataset.

If you want to override the path (e.g. to split a single workflow into multiple files), pass `data-file-path` explicitly:

```yaml
- uses: hatsu38/ghtrack@<sha>
  with:
    data-file-path: data/custom-name.json
```

In all cases, `data/manifest.json` is auto-generated on `gh-pages` and tracks every registered path:
...
On every run, the Action bundles `index.html` at the root of the `gh-pages` branch (auto-syncs if it changed). It loads Chart.js v4 + the date-fns adapter from a CDN (jsDelivr), fetches the data via the paths listed in `./data/manifest.json`, and renders two time-series charts:
...
```bash
# switch to the gh-pages branch (so index.html and data/*.json are at the root)
git switch gh-pages
...
- **Concurrent push contention**: parallel updates to the same data file are detected by optimistic locking on `sha` and retried with exponential backoff (up to 5 times). Setting `concurrency` on the same workflow / branch is the safer way to prevent contention from happening in the first place
